...
IMAGE_TAG="$IMAGE_TAG" docker compose -f "$COMPOSE_FILE" pull "$SERVICE"

# Capture current container ID for rollback
OLD_CONTAINER=$(docker compose -f "$COMPOSE_FILE" ps -q "$SERVICE" 2>/dev/null || true)

echo "==> [deploy] Restarting ${SERVICE} with new image"
IMAGE_TAG="$IMAGE_TAG" docker compose -f "$COMPOSE_FILE" up -d --no-deps "$SERVICE"
